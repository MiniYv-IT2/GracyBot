...
from gracybot.core.gracy_adapter.satori.protocol import SatoriOpcode, parse_satori_event
from gracybot.core.gracy_adapter.satori.event import satori_event_to_gracy
...
class SatoriGateway:
    ...
    def __init__(
        ...
        port: int,
        token: str,
        on_event: Callable[[GracyEvent], None],
        tag: IdentityTag,
    ):
        self._host = host
        self._port = port
    ...
    async def call_api(self, action: str, params: dict = None) -> dict:
        ...
        import uuid
        echo = f"gracy_{uuid.uuid4().hex[:12]}"
    ...
    async def _handle_message(self, data: dict):
        ...
        if op == SatoriOpcode.EVENT.value:
            ...
            if event_data:
                event = satori_event_to_gracy(event_data, self._tag)
