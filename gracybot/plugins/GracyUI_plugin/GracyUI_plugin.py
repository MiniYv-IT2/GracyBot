...
def _start_quart():
    ...
    for attempt in range(5):
        try:
            from gracybot.plugins.GracyUI_plugin.backend.app import create_app
            app = create_app()
            ...
            logger.info(f"Quart 正在监听 http://0.0.0.0:{_PANEL_PORT}")
            _loop.run_until_complete(serve(app, cfg))
            return
        except OSError as e:
            logger.warning(f"Quart 端口 {_PANEL_PORT} 被占用，第{attempt+1}次重试...")
            _time.sleep(2)
        except Exception as e:
            logger.error(f"Quart 启动失败: {e}")
            return
    logger.error(f"Quart 启动失败: 端口 {_PANEL_PORT} 5次重试后仍不可用")
...
def _boot_panel_and_notify():
    ...
    try:
        ...
        if _is_first_run:
            logger.info("master_id 未配置，跳过发送面板地址（请先编辑 config.json 填写 master_id）")
            logger.info(f"Quart 正在监听 http://0.0.0.0:{_PANEL_PORT}")
            return
        ...
        try:
            ...
            logger.warning(f"调度发送失败: {_e}")
        logger.info(f"✔ 面板已自启，地址已发送给主人")
    except Exception as e:
...
def start_panel_boot():
    ...
    if not os.environ.get("GRACY_NO_WEBUI"):
        ...
        logger.info("GRACY_NO_WEBUI 已设置，跳过自启面板")
...
async def handle_gracy_ui(plugin_manager, send_msg, data, sender_id, chat_type, permission, logger):
    logger.info(f"接收到命令，发送者: {sender_id}")
    try:
...
async def _start_or_get_url(send_msg, sender_id, chat_type, logger):
    ...
    if _quart_thread is None or not _quart_thread.is_alive():
        logger.info("首次请求，启动 Quart 面板服务...")
        _quart_thread = threading.Thread(target=_start_quart, daemon=True, name="GracyUI-Quart")
        _quart_thread.start()
        import time as _t
