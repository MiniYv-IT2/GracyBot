...
def _screenshot_page(url: str, wait_ms: int = 3000) -> Optional[str]:
    cfg = _load_config()
    ...
    try:
        ...
        page.screenshot(path=path, full_page=False, clip={
            "x": 0, "y": 0,
...
def do_search(query: str, engine: str = None) -> Dict:
    cfg = _load_config()
    if not engine:
