            ...
            if is_master:
                ...
                def delayed_startup():
                    ...
                    try:
                        ...
                        # Python原生开机方式 - 启动新的机器人进程
                        if platform.system() == "Windows":
                            # Windows平台启动
                            subprocess.Popen([sys.executable] + sys.argv, creationflags=subprocess.CREATE_NEW_CONSOLE)
                        else:
                            # Linux/Unix/Mac平台启动 - 使用nohup确保后台运行
...
• 反馈建议：欢迎到插件社区提交 Issue 或 PR"""
        ...
        if not handled:
            try:
                ...
                is_private_direct_chat = chat_type == "private" and raw_msg.strip() and not (
                    ...
                    if is_auto_reply_match:
                        ...
                        try:
                            ...
                            if config.get("api_key"):
                                ...
                                logger.debug(f"[任务提取] 消息: {raw_msg[:30]}... | 提取结果: {task_info}")
                                if task_info:
                                    ...
                                    # 让AI知道准确的任务时间，生成友好的确认回复
                                    enhanced_msg = f"{raw_msg}\n[系统提示：已计算出准确的提醒时间为 {task_info['time']}，请在回复中使用这个时间]"
                                    handle_ai_chat(bot_sender, target_id, chat_type, enhanced_msg, sender_id, nickname, chat_id)
                                    handled = True
                                    auto_reply = None
                                else:
                                    logger.debug(f"[任务提取] 未检测到定时任务，调用AI对话")
                                    # 调用AI处理
                                    handle_ai_chat(bot_sender, target_id, chat_type, raw_msg, sender_id, nickname, chat_id)
                                    handled = True
                                    auto_reply = None
                            else:
                                auto_reply = None
...
def process_event_from_adapter(event: "GracyEvent") -> dict | None:
    ...
    # 记录收到消息（优先纯文本，空则回退原始 CQ 码 → 由 sanitize 转为 [图片]/[文件] 等）
    simplified_context = {
