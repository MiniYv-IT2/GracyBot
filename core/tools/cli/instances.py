"""实例管理 — gracy instance <list|add|enable|disable|remove>"""
...
instance_cli = typer.Typer(help="实例管理（style/instances/<name>/）")


def _instances_dir() -> Path:
    """返回 style/instances 目录"""
    base = os.environ.get("GRACYBOT_HOME", os.getcwd())
    return Path(base) / "style" / "instances"
...
def cmd_list():
    ...
    if not instances:
        typer.echo("  ℹ️  没有实例（style/instances/<name>/config.json）")
        typer.echo("  💡 使用 gracy instance add <name> 创建")
        return
...
def cmd_add(
    ...
    typer.echo("  💡 重启生效: gracy stop && gracy run")
