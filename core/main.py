...
from core.tools.log_runtime import setup_runtime_logger
from core.pipeline import Pipeline, SecurityFilter, BuiltinCommands, CommandMatcher, PluginHandler, ResponseSender
    ...
    if http_port:
        try:
            ...
            try:
                ...
                if master_id:
                    asyncio.create_task(
                        gracy_send_msg(master_id, GracyText(text=f"❌ 机器人启动失败\n错误: {str(e)}"), chat_type="private")
                    )
            except Exception:
                pass
            sys.exit(1)
